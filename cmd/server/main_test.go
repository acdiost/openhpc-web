...
func TestParseTerminalConfigFromStore(t *testing.T) {
	...
	defer store.Close()
	if err := store.Set(context.Background(), "OPENHPC_TERMINAL_SSH_ADDRESS", "configured-login.example:2222"); err != nil {
		t.Fatal(err)
	}

	enabled, config, err := parseTerminalConfigFromStore(store)
	if err != nil {
		t.Fatal(err)
	}
